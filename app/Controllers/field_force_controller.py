...
from app.Services.Automation.field_force_excel import process_field_force_excel, generate_ff_sample # ডুপ্লিকেট ইম্পোর্ট ফিক্সড ✅
...
async def handle_ff_house_logic(message: Message, state: FSMContext, user_tg_id, is_callback=False):
    async with async_session() as session:
        ...
        # যদি মাত্র ১টি হাউজ থাকে এবং সেটি সুপার এডমিন না হয় (সরাসরি মেইন ড্যাশবোর্ড) ✅
        if len(target_houses) == 1 and not is_super_admin:
...
async def render_ff_main(message: Message, house_id: int, is_callback=False):
    ...
    async with async_session() as session:
        ...
        total_count = await session.scalar(select(func.count(FieldForce.id)).where(FieldForce.house_id == house_id))
        ...
        text = (f"🏪 **ফিল্ড ফোর্স ম্যানেজমেন্ট**\n🏢 হাউজ: **{house.name}**\n\n📊 এই হাউজে মোট **{bn_num(total_count)}** জন মেম্বার আছে।")

        kb = get_field_force_main_kb(house_id, total_count, kb_permissions, is_super_admin, personal_ff_id)

        if is_callback or hasattr(message, 'edit_text'):
...
async def back_to_ff_main(callback: CallbackQuery):
    ...
    await render_ff_main(callback.message, house_id, is_callback=True)
    await callback.answer()
...
async def handle_ff_list(callback: CallbackQuery):
    ...
    async with async_session() as session:
        ...
        res = await session.execute(
            select(FieldForce).where(FieldForce.house_id == house_id).order_by(FieldForce.name).offset(offset).limit(limit)
        )
        ...
        # কিবোর্ড ফাইলে m.itop_number ব্যবহার নিশ্চিত করতে হবে ✅
...
async def view_ff_profile(callback: CallbackQuery):
    ...
    async with async_session() as session:
        ...
        if is_super_admin: perms.append("delete_field_force")
        
        await callback.message.edit_text(
            get_field_force_full_profile_text(m), 
            reply_markup=get_ff_action_kb(m.id, m.house_id, m.status, perms), 
            parse_mode="HTML"
        )
    await callback.answer()
...
async def show_edit_categories(callback: CallbackQuery):
    ...
    if "ff_ecat_" in callback.data:
        ...
        await callback.message.edit_text("কোন তথ্যটি আপডেট করতে চান?", reply_markup=get_ff_fields_by_category_kb(category_key, ff_id))
    else:
...
async def process_field_selection(callback: CallbackQuery, state: FSMContext):
    ...
    async with async_session() as session:
        ...
        await callback.message.answer(f"📂 ফিল্ড: **{field_name.replace('_', ' ').capitalize()}**\n📌 বর্তমান: `{display_value}`\n\nনতুন তথ্যটি লিখে পাঠান:", parse_mode="Markdown")
...
async def save_ff_edit(message: Message, state: FSMContext):
    ...
    async with async_session() as session:
        ...
        setattr(m, data['edit_field'], message.text.strip())
        await session.commit()
        ...
        await message.answer(get_field_force_full_profile_text(m), reply_markup=get_ff_action_kb(m.id, m.house_id, m.status, perms), parse_mode="HTML")
    await state.clear()
...
async def start_ff_search(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer("🔍 নাম অথবা ডিএমএস কোড লিখে পাঠান:")
    await state.set_state(FFStates.searching)
    await callback.answer()

# --- সার্চ লজিক আপডেট (এখন আইটপ নাম্বার দিয়েও সার্চ করা যাবে) ---
        ...
        res = await session.execute(
            select(FieldForce).where(
                ...
                or_(
                    ...
                    FieldForce.dms_code.icontains(query),
                    FieldForce.itop_number.icontains(query) # আইটপ নাম্বার দিয়ে সার্চ ✅
                )
            )
        )
        results = res.scalars().all()
        if not results: return await message.answer("❌ পাওয়া যায়নি।")
        builder = InlineKeyboardBuilder()
        for r in results: builder.button(text=f"👤 {r.name} ({r.dms_code})", callback_data=f"ff_view_{r.id}")
        await message.answer(f"🔍 রেজাল্ট ({len(results)} জন):", reply_markup=builder.adjust(1).as_markup())
    await state.clear()

@router.callback_query(F.data.startswith("ff_upload_"))
async def start_excel_upload(callback: CallbackQuery, state: FSMContext):
    await state.update_data(upload_house_id=int(callback.data.split("_")[2]))
    await callback.message.answer("📁 এক্সেল ফাইলটি (.xlsx) পাঠান।")
...
async def handle_excel_upload(message: Message, state: FSMContext):
    if not message.document.file_name.endswith('.xlsx'): return await message.answer("❌ শুধু .xlsx ফাইল দিন।")
    data = await state.get_data()
    ...
    if err: await wait_msg.edit_text(f"❌ এরর: {err}")
        ...
        await wait_msg.edit_text(f"✅ সম্পন্ন! `{bn_num(count)}` টি রেকর্ড আপডেট হয়েছে।")
        await asyncio.sleep(2)
        await render_ff_main(wait_msg, house_id)
    await state.clear()

@router.callback_query(F.data.startswith("ff_del_conf_"))
async def confirm_delete(callback: CallbackQuery):
    ff_id = int(callback.data.split("_")[3])
    async with async_session() as session:
        m = await session.get(FieldForce, ff_id)
        builder = InlineKeyboardBuilder()
        builder.button(text="✅ নিশ্চিত", callback_data=f"ff_del_final_{ff_id}")
        builder.button(text="❌ না", callback_data=f"ff_view_{ff_id}")
        await callback.message.edit_text(f"⚠️ **{m.name}** কে ডিলিট করবেন?", reply_markup=builder.adjust(2).as_markup())
...
async def final_delete(callback: CallbackQuery):
    ...
    async with async_session() as session:
        ...
        if m:
            ...
            await callback.answer("🗑 মুছে ফেলা হয়েছে।", show_alert=True)
            await render_ff_main(callback.message, h_id)
    await callback.answer()
