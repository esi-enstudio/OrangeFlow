...
async def retailer_main(event: types.Union[Message, CallbackQuery], state: FSMContext, permissions: list):
    ...
    async with async_session() as session:
        ...
        if int(user_id) == int(SUPER_ADMIN_ID):
            ...
            for h in all_houses: builder.button(text=f"🏢 {h.name}", callback_data=f"ret_hsel_{h.id}")
        ...
        builder = InlineKeyboardBuilder()
        for h in user.houses: builder.button(text=f"🏢 {h.name}", callback_data=f"ret_hsel_{h.id}")
